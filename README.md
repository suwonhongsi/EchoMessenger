...
- 사용한 기술과 구현한 기능
  - `TextBox.Text` 속성을 참조하여 사용자가 입력한 문자열 데이터를 동적으로 추출하는 기술을 사용했습니다. 또한 `ListBox.Items.Add()` 메소드를 활용하여 정적 배열이 아닌 동적 리스트 형태로 데이터를 누적시키는 기능을 구현하였으며, 전송 완료 후 `Clear()` 기능을 통해 입력 버퍼를 초기화하는 기술을 적용했습니다.
...
- 사용한 기술과 구현한 기능
  - `Focus()` 메소드를 이용한 커서 제어 기술과 `KeyDown` 이벤트 매핑 기술을 사용했습니다. 특히 엔터 키 입력을 감지하기 위해 `e.KeyCode`를 판별하는 로직을 구현하였으며, `string.IsNullOrWhiteSpace()` 정적 메소드를 활용해 데이터 유효성을 검증하고 무분별한 공백 데이터 전송을 차단하는 기능을 적용했습니다.
...
- 사용한 기술과 구현한 기능
  - `DateTime.Now` 클래스와 `ToString()` 포맷 지정 기술을 사용했습니다. 또한 C#의 보간 문자열(`$""`) 기술을 활용해 시간과 텍스트를 결합하는 기능을 구현했으며, `ListBox.Items.Count` 속성을 활용한 실시간 데이터 모니터링 기능 및 `string.Trim()`을 이용한 데이터 전처리 기술을 적용했습니다.
...
- 사용한 기술과 구현한 기능
  - `SelectedIndex` 판별을 통한 예외 처리 기술과 `Items.RemoveAt()` 컬렉션 제어 기술을 사용했습니다. 리스트 박스 전체 데이터를 비우기 위해 `Items.Clear()` 기능을 구현하였으며, TextBox의 `MaxLength` 속성 설정 및 조건문을 통한 입력 글자 수 제한 기능을 적용하여 시스템 안정성을 확보했습니다.

---

## 배운 내용
- **컨트롤 이벤트 핸들링**: 버튼 클릭 외에도 키보드 이벤트를 통해 사용자 편의성을 크게 개선할 수 있다는 점을 배웠습니다.
- **문자열 데이터 가공**: 사용자의 날것(Raw) 데이터를 `Trim()`이나 `IsNullOrWhiteSpace()` 등을 통해 정제하는 과정이 소프트웨어의 완성도를 높인다는 것을 깨달았습니다.